...
/* GX addition: expose the internal HID v5 host fd so the GameCube
 * adapter code can issue raw ioctls on the handle that the resource
 * manager actually talks to. */
s32 USB_GetHidHostFd(void)
{
	return hid_host ? hid_host->fd : -1;
}
